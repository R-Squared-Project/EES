...
import { HashLockValidationError, NativeAccountValidationError } from "context/Domain/Errors";
    ...
    describe("execute", () => {
        ...
        describe("error", () => {
            ...
            it("should return error if hashLock is invalid", async () => {
                ...
                await expect(handler.execute(command)).rejectedWith(
                    HashLockValidationError,
                    'HashLock "invalid_hashLock" is invalid: HashLock format is invalid'
                );
            });
