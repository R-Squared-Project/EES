...
describe('ProcessIncomeContractCreationHandler', () => {
    ...
    beforeEach(function() {
        ...
        handler = new RedeemDepositExternalContractHandler(
            depositRepository,
            externalBlockchain
        );
    });
    ...
    describe('execute', () => {
        describe('success', () => {
            ...
            it('should use send correct request to an external blockchain', async () => {
                ...
                expect(redeemRequest.secret).equals(`0x${secret}`)
            })
        })
        ...
        describe('error', () => {
            ...
            it('should throw error if deposit status is invalid (and secret is empty)', async () => {
                ...
                const command = new RedeemDepositExternalContract(deposit.idString)
                await expect(handler.execute(command)).rejectedWith(ErrorsDomain.RedeemExecutedInExternalBlockchainStatusError)
            })
        })
    });
});
