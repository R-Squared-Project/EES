...
export class AlterDepositEntityAddRelations1669286186388 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(
            `CREATE TABLE \`external_contract\` (\`id\` varchar(255) NOT NULL, \`status\` int NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`
        );
        ...
        await queryRunner.query(
            `ALTER TABLE \`deposit\` ADD UNIQUE INDEX \`IDX_579bbfff76624a8e2bfdbf8d66\` (\`deposit_request_id\`)`
        );
        await queryRunner.query(`ALTER TABLE \`deposit\` ADD \`external_contract_id\` varchar(255) NULL`);
        await queryRunner.query(
            `ALTER TABLE \`deposit\` ADD UNIQUE INDEX \`IDX_e0920857a2b620f4541617c8d7\` (\`external_contract_id\`)`
        );
        await queryRunner.query(
            `CREATE UNIQUE INDEX \`REL_579bbfff76624a8e2bfdbf8d66\` ON \`deposit\` (\`deposit_request_id\`)`
        );
        await queryRunner.query(
            `CREATE UNIQUE INDEX \`REL_e0920857a2b620f4541617c8d7\` ON \`deposit\` (\`external_contract_id\`)`
        );
        await queryRunner.query(
            `ALTER TABLE \`deposit\` ADD CONSTRAINT \`FK_579bbfff76624a8e2bfdbf8d662\` FOREIGN KEY (\`deposit_request_id\`) REFERENCES \`deposit_request\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`
        );
        await queryRunner.query(
            `ALTER TABLE \`deposit\` ADD CONSTRAINT \`FK_e0920857a2b620f4541617c8d74\` FOREIGN KEY (\`external_contract_id\`) REFERENCES \`external_contract\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`
        );
    }
