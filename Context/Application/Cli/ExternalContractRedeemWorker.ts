...
export class ExternalContractRedeemWorker extends CommandRunner {
    constructor(private queue: RabbitMQ, private handler: ConfirmDepositExternalContractRedeemedHandler,) {
        super()
    }

    async run(): Promise<void> {
        this.queue.consume<ExternalContractRedeemMessage>(this.queue.EXTERNAL_CONTRACT_REDEEM, async (message: ExternalContractRedeemMessage, ack, nack) => {
